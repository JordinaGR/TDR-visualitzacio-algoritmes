...
def database(algtype, alg, size, sec, sdata, speed):
    ...
    if algtype == 'sort':
        dfso = '/home/jordina/Desktop/tdr/sorting-algs-visualization/db/sortdata.csv'
        with open(dfso, 'a') as df:
            w = csv.writer(df)
            w.writerow([alg, size, sec, speed])

    elif algtype == 'search':
        dfse = '/home/jordina/Desktop/tdr/sorting-algs-visualization/db/searchdata.csv'
...
def Start_alg():
    ...
    if alg_menu.get() == "Bombolla":     # if buble sort selected:
        ...
        sorting_algs_func('bombolla', end, start, size, speed)
    ...
    elif alg_menu.get() == "Barreja":
        start = time.perf_counter()
        merge_sort(data, drawdata, speed)
        end = time.perf_counter()
        sorting_algs_func('Barreja', end, start, size, speed)
...
# ui
alg_menu = Combobox(root, width=15, textvariable=selected_alg, values=['Algs' , 'Bombolla', 'Barreja', 'Opti Bubble Sort', 'Bogo Sort', 'Selection Sort', 'Insertion Sort', 'Quick Sort', 'Shell Sort', 'Counting Sort', 'Radix Sort', 'Cocktail Sort'])
alg_menu.place(x=290, y=20)
alg_menu.current([0])
ser_menu = Combobox(root, width=15, textvariable=selected_search, values=['Search Alg' ,'Lineal', 'Dicotomica', 'Exponential Search'])
